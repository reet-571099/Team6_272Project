		...
		const fetchProjectTasks = async () => {
			try {
				const username =
					...
					"nikhilkoli287@gmail.com";

				const response = await fetch(
					`http://3.15.28.161:5001/get_all_issues_in_project?username=${encodeURIComponent(
						username
					)}&project_key=${encodeURIComponent(project.key)}`
				);
	...
	useEffect(() => {
		const fetchProjects = async () => {
			try {
				...
				const response = await fetch(
					`http://3.15.28.161:5001/get_all_project_keys?username=${username}`
				);
...
const Dashboard = () => {
	...
	const [showJiraModal, setShowJiraModal] = useState(() => {
		...
		const userDetailsFromCookies = cookie.find((row) =>
			row.startsWith("user_obj=")
		);
	...
	const pollActiveStories = async (userId, projectId) => {
		try {
			const response = await axios.get(
				`http://54.193.65.42:3000/api/getActiveStories`,
				{
					params: {
						user_id: userId,
	...
	const fetchStories = async (projectId) => {
		try {
			const response = await axios.get(
				`http://54.193.65.42:3000/api/stories/${projectId}`
			);

			navigate("/tasks", {
				state: {
					tasks: response.data.stories,
		...
		try {
			...
			if (!token) {
				...
				if (jwtCookie) {
					...
					throw new Error(
						"No JWT token found in cookies. Please log in again."
					);
				}
			}
			...
			const response = await axios.post(
				"http://54.193.65.42:8000/api/upload",
				formData,
				{
					headers: {
			...
			if (response.status === 200) {
				const pollInterval = setInterval(async () => {
					const activeStoriesCount = await pollActiveStories(
						userid,
						selectedProject.key
					);
